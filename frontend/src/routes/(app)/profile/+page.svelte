<script lang="ts">
  ...
  function getErrorMessage(error: any): string {
    ...
    if (data.message?.includes("file size")) {
      const match = data.message.match(
        /current file size is (\d+), expected size is less than (\d+)/,
      );
      if (match) {
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      toaster.create({
        title: $t("common.error"),
        description: error.response?.data?.message || $t("profile.updateError"),
        type: "error",
      });
    } finally {
...
{#if !auth.isAuthenticated && authChecked}
  <div class="min-h-[calc(100vh-120px)] flex items-center justify-center px-4">
    <div class="text-center space-y-4 animate-fade-in">
      <div
        class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-primary-500/20 mb-2"
      >
        <LoaderCircle size={32} class="text-primary-400 animate-spin" />
      </div>
...
{:else}
  <div class="min-h-[calc(100vh-120px)] px-4 py-6 sm:px-6 lg:px-8">
    <div class="max-w-2xl mx-auto">
      <div
        class="bg-surface-900/80 backdrop-blur-sm rounded-2xl border border-surface-800/50 overflow-hidden"
      >
        <div class="relative">
          <div
            class="h-24 sm:h-32 bg-linear-to-br from-primary-600/30 via-primary-700/20 to-surface-900"
          >
            <div
              class="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(var(--color-primary-500)/0.15),transparent_50%)]"
            ></div>
          </div>
          ...
          <div class="relative -mt-12 sm:-mt-14 px-4 sm:px-6">
            ...
            >
              <div class="relative shrink-0 group">
                {#if isUploadingAvatar}
                  <div
                    class="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl ring-4 ring-surface-900 shadow-xl bg-surface-800 flex items-center justify-center"
                  >
                    <LoaderCircle
                      size={32}
              ...
              <div class="flex-1 pb-3 text-center sm:text-left">
                ...
                <div
                  class="flex flex-wrap items-center justify-center sm:justify-start gap-3 mt-1.5"
                >
                  <div
                    class="flex items-center gap-1.5 text-xs font-semibold text-surface-400"
                  >
                  ...
                  <div
                    class="flex items-center gap-1.5 text-xs font-medium text-surface-500"
                  >
                    <Calendar size={14} />
                    <span>
            ...
            <div
              class="flex items-center justify-center sm:justify-start gap-2 mt-4 pb-4 sm:hidden"
            >
              <label
        ...
        <form onsubmit={handleSubmit}>
          <div class="px-4 sm:px-6 pb-6 space-y-5">
            <div
              class="flex items-center gap-2 pt-2 border-t border-surface-800/50"
            >
              <h2
                class="text-[10px] font-black uppercase tracking-[0.15em] text-surface-500 py-3"
              >
                {$t("profile.info")}
              </h2>
            </div>
